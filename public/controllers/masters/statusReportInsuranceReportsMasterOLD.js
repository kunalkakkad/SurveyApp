...
//                            console.log('True');
                ...
                STSData.statusReportRetriever($rootScope.statusReport.Main._id)
                    .then(function(res){
                        console.log('Status Report Received',res);
                        $rootScope.statusReport.Status = res.data[0];
                    },function(error){
                ...
                STSData.statusReportDamageDetailsRetriever($rootScope.statusReport.Main._id)
                    .then(function(res){
                        console.log('Received status report damage details',res);
                        $rootScope.statusReport.Status_Damage_Details = angular.copy(res.data);
                        $rootScope.$broadcast('statusReportDamageDetailsReceived');
                    },function(error){
                        console.error('An Error ocured while retrieving the Status Report Damage Details',error);                                        
                    })
                    .catch(function(error){
                        console.log('An Error ocured while retrieving the Status Report Damage Details',error);                                        
                    })
            });
    ...
    if($rootScope.statusReportDataLoadedOnce == false){
        SRVData.surveysRetriever('Final')
            ...
            .then(function(){
                SRVData.surveysRetriever('Interim')
                    .then(function(res){
